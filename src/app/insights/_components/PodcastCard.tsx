...
export function PodcastCard({ item, isMobile = false }: PodcastCardProps) {
  ...
  return (
    <div className="podcast-card-clip relative bg-white p-0.5">
      <div className="podcast-card-clip-inner bg-[#F6F8FB] p-4 mobile:p-5">
        <div className="flex items-center justify-between gap-3">
          <h3 className="line-clamp-1 flex-1 font-poppins text-xl/8 font-bold mobile:text-base/6">{item.title}</h3>
          <img
            src="/imgs/podcast/podcast-play-white.svg"
            className="h-10 w-10 shrink-0 cursor-pointer duration-300 hover:scale-110 mobile:size-11"
            alt="Play"
            onClick={handlePlay}
          />
        </div>
        <p className="line-clamp-1 text-base font-medium mobile:text-sm">{item.description}</p>
      </div>
    </div>
  );
}
