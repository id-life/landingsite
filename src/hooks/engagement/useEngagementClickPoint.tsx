...
export function useEngagementClickPoint(needTrack = true) {
  ...
  const trackEngagementEvent = useCallback(
    ...
    [needTrack, trackEvent],
  );

  const handleClickPoint = useCallback(
    (type: EngagementClickPointType, index: number, openState?: boolean) => {
      if (type === 'meeting') {
        ...
        newState !== null && trackEngagementEvent(type, newState, 'click');
      } else if (type === 'book') {
  ...
  const handleMouseEnterCallback = useCallback(
    (e: React.MouseEvent, index: number, type: EngagementClickPointType) => {
      e.stopPropagation();
      if (type === 'meeting') {
  ...
  const handleMouseEnter = useThrottle(handleMouseEnterCallback, 400);
  ...
  const handleMouseLeaveCallback = useCallback(
    ...
    [activeMeetingDot, activeBookDot, activeSponsorDot, setActiveMeetingDot, setActiveBookDot, setActiveSponsorDot],
  );

  const handleMouseLeave = useThrottle(handleMouseLeaveCallback, 400);
