try {
  ...
  if (typeof p5 !== 'undefined') {
    // 这里放置您的 particle-gl.js 代码
  } else {
    throw new Error('p5 is not defined. Make sure p5.js is loaded before particle-gl.js');
  }
  ...
  var imgs = [];
  var imgNames = ['/imgs/id-logo.png'];

  const IS_MOBILE = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  var scaleNum = IS_MOBILE ? 1.3 : 2.2;
  var imgIndex = 0;
  var loadPercentage = 0.0007; // 0 to 1.0
  var closeEnoughTarget = 100;
  let resolution = IS_MOBILE ? 20 : 5;
  let speed = 3;
  let particleSize = IS_MOBILE ? 6 : 8;
  let mouseSize = 50;
  let scaleRatio = 1;
  ...
  function preload() {
    ...
    for (var i = 0; i < imgNames?.length; i++) {
      ...
      newImg.resize(256, 150);
      imgs.push(newImg);
    }
  }

  function setup() {
    const canvas = createCanvas(1280, 750);
